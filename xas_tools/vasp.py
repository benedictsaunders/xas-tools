...
__maintainer_email__ = (
    "hg2568@columbia.edu, nartrith@atomistic.net" + ", a.urban@columbia.edu"
)
__date__ = "2023-10-31"
__version__ = "0.1.1"
...
class CHPCalculation(object):
    def __init__(
        ...
        Args:
          ...
          z: electron count, i.e., for fractional core holes is shielding
            is considered
          U: dictionary of hubbard corrections to be considered in calculations
            in the form {"Fe":{"L":2, "U":4.0, "J":0.0}}
          m: dictionary of magnetic moments in the form
            {"Fe":5.0, "O":0.6}
        ...
        for i, atom in self.atoms:
            ...
            for so in symmops[1:]:
                ...
                distances = self.structure.lattice.get_all_distances(
                    [symm_coo1], all_coords
                )[0]
                equiv = [j for j in range(len(self.atoms)) if distances[j] <= 1.0e-3]
                for atom2 in equiv:
    ...
    def write_vasp_input(
        ...
        vaspset = DictSet(struc_super, config, sort_structure=False, force_gamma=True)

        # number of valence electrons for each atomic species,
        # identified by the atomic number
        num_elec = {
            p.atomic_no: np.sum([n for _, _, n in p.electron_configuration])
            for p in vaspset.potcar
        }

        # total number of valence electrons
        num_valence = np.sum([num_elec[s.specie.number] for s in vaspset.structure])
        if self.hubb is not None:
            ldauu, ldaul, ldauj = {}, {}, {}
            for elem, params in self.hubb.items():
                ldaul[elem] = params["L"]
                ldauu[elem] = params["U"]
                ldauj[elem] = params["J"]
            ldau = {
        ...
        if write_no_ch:
            ...
            vaspset.user_incar_settings.update(
                {"NBANDS": num_valence * band_multiple, "NCORE": ncore}
            )
            if self.hubb is not None:
                vaspset.user_incar_settings.update(ldau)
            if self.mgm is not None:
                vaspset.user_incar_settings.update({"MAGMOM": self.mgm})
            vaspset.write_input(
                os.path.join(path, "XAS_input_SCF"), make_dir_if_not_present=True
            )

        w1 = len(str(max(self.active_atom_types)))
        w2 = len(str(max(self.active_mult.values())))
        dir_frmt = "XAS_input_{:0" + "{}".format(w1) + "d}_{:0" + "{}".format(w2) + "d}"
        ...
        metadata = {
            ...
            "multiplicity": [i.tolist() for i in list(self.active_mult.values())],
        }
        ...
        for t in self.active_atom_types:
            ...
            active = [
                i for i, s in enumerate(struc_super) if s.properties["ch_select"] == t
            ]
            ...
            xas_struc = Structure.from_sites(site_list)

            vaspset = DictSet(xas_struc, config, sort_structure=False, force_gamma=True)
...
def parse_vasp_chp_output(base_path, output_path="XAS_output"):
    ...
    if not os.path.exists(output_path):
        ...
        outdir = os.path.join(output_path, "structure_{}".format(1))
    else:
        num_old = len(glob.glob(os.path.join(output_path, "structure_*")))
        outdir = os.path.join(output_path, "structure_{}".format(num_old + 1))
    os.mkdir(outdir)
    ...
    structure_no_ch = Structure.from_file(os.path.join(no_ch_path, "CONTCAR"))
    structure_ch = Structure.from_file(os.path.join(ch_paths[0], "CONTCAR"))
    ...
    shutil.copyfile(
        os.path.join(no_ch_path, "CONTCAR"), os.path.join(outdir, "no_corehole.vasp")
    )
    ...
    chp_path_frmt = "atom_{:0" + "{}".format(w1) + "d}_{:0" + "{}".format(w2) + "d}"
    ...
    for i, path in enumerate(ch_paths):
        ...
        with monty.io.zopen(outcar_path) as fp:
            ...
            while len(line.strip()) > 0:
                ...
                xas.append([dielectric[0], dielectric[0] + dE, np.sum(dielectric[1:])])
                line = fp.readline()
                if hasattr(line, "decode"):
                    line = line.decode("utf-8")
        xas = np.array(xas)
        ...
        df = pd.DataFrame(
            data=xas, columns=["Raw Energy (eV)", "Aligned Energy (eV)", "Intensity"]
        )
        ...
        shutil.copyfile(
            os.path.join(path, "CONTCAR"), os.path.join(outdir, poscar_path)
        )
